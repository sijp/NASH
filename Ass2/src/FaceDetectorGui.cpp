...
FaceDetectorGui::FaceDetectorGui(const string &imgName ,const string &linuxTitle) : detector(imgname), imgTitle(linuxTitle)
{
...
bool FaceDetectorGui::show()
{
  ...
  for ( i = 0 ; i<this->faces.size() ; i++ )
  {
    ...
    rectangle(this->result, p1, p2, Scalar(255 , 0 , 255) , 200);
  }
  
  imshow(this->imgTitle , this->result);
  return true;
}
