...
int main (int argc, char *argv[]) {
    ...
    EchoClient EchoClient(host, port);
    if (!EchoClient.connect()) {
	...
	if (!EchoClient.sendBytes(utf8g.c_str(),strlen(utf8g.c_str()))) {
			std::cout << "Disconnected. Exiting...\n" << std::endl;
			return 1;
	}
	//get the echo back from the server as simple bytes:
	char greekEcho[256];
	if (!EchoClient.getBytes(greekEcho, strlen(utf8g.c_str()))) {
			std::cout << "Disconnected. Exiting...\n" << std::endl;
			return 1;
	}
    ...
    while (1) {
        ...
        // We could also use: EchoClient.getline(answer) and then get the answer without the newline char at the end
        if (!EchoClient.getBytes(answer, len+1)) {
            std::cout << "Disconnected. Exiting...\n" << std::endl;
            break;
        }
		...
		// strlen(answer) counts the chars until the 0 char is found.
        answer[len] = 0;
        std::cout << "Reply: " << answer << " " << strlen(answer) << " bytes " << std::endl;
        if (std::string(answer) == "bye") {
            std::cout << "Exiting...\n" << std::endl;
            break;
        }
    }
